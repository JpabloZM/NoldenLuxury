...
export async function PUT(
  ...
  try {
    ...
    const newInventory = body.inventory !== undefined ? body.inventory : currentInventory;
    ...
    if (inventoryChanged !== 0 && inventoryChanged > 0) {
      console.log(`Product ${id} inventory increased by ${inventoryChanged}. Processing material discount.`);
      ...
      try {
        ...
        if (recipesError) {
          ...
          for (const recipe of recipes) {
            ...
            const current_quantity = Math.floor(Number(materialData?.quantity || 0));
            ...
            if (updateMaterialError) {
              console.error(`Error updating material ${material_id}:`, updateMaterialError);
              continue;
            }
            ...
            if (movementError) {
              console.error(`Error creating material movement for ${material_name}:`, movementError);
            } else {
              console.log(`Material movement recorded for ${material_name}`);
            }
          }
        } else {
          console.log(`No recipes found for product ${id} - inventory will be updated without material discount`);
        }
      } catch (err) {
